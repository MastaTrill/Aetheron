...
const { ERC20Token, ERC721Token } = { ERC20Token: class {}, ERC721Token: class {} }; // Placeholder classes
...
function prompt() {
  rl.question('\nAetheron CLI> ', async (input) => {
    ...
    case 'send': {
      ...
      tx.signTransaction(
        wallet.encrypted ? wallet.getDecryptedPrivateKey(password) : wallet.privateKey
      );
    ...
    case 'propose': {
      ...
      const id = global.governance
        ? global.governance.propose(desc)
        : ((global.governance = new (require('./governance').Governance())()),
        global.governance.propose(desc));
